// ---------------------------------------------------------------------------
// server/toc-kompas/ddl.ts
//
// De tabeldefinitie van het TOC Commitmentkompas, op een plaats. Dezelfde tekst
// staat letterlijk in migrations/0013_toc_kompas.sql (vanaf de eerste
// CREATE-regel); tests/toc-kompas-migratie.test.ts bewaakt dat beide gelijk
// blijven en dat server/toc-kompas/schema.ts dezelfde kolommen beschrijft.
//
...
//
// ZES TABELLEN. Een ronde per kwartaal, de vier Captains van die ronde met hun
// persoonlijke link (enkel de sha256-hash van het token), de ingediende
// vragenlijsten, het Commitment Register, het Decision Log en de bewaarde
// rapportversies.
//
// ONVERANDERLIJKHEID. Triggers dwingen in de databank zelf af wat de toepassing
// ook afdwingt: een ingediende vragenlijst wordt nooit gewijzigd (een nieuwe
// indiening is een nieuwe rij met een hogere versie), een besluit wordt nooit
// gewijzigd of verwijderd (een correctie is een nieuw besluit dat het vorige
// vervangt), en een rapportartefact wordt nooit gewijzigd.
// ---------------------------------------------------------------------------

export const TOC_KOMPAS_DDL = `CREATE TABLE IF NOT EXISTS \`toc_kompas_rondes\` (
...
export function tocKompasStatements(): string[] {
  return TOC_KOMPAS_DDL.split("--> statement-breakpoint")
    .map((s) => s.trim())
    .filter((s) => s.length > 0);
}

export const TOC_KOMPAS_TABELLEN = [
  "toc_kompas_rondes",
  "toc_kompas_captains",
  "toc_kompas_indieningen",
  "toc_kompas_commitments",
  "toc_kompas_besluiten",
